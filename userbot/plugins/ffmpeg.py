...
from userbot.plugins import media_type, progress, reply_id

FF_MPEG_DOWN_LOAD_MEDIA_PATH = "./downloads/catuserbot.media.ffmpeg"


@icssbot.on(admin_cmd(pattern="ffmpegsave$"))
@icssbot.on(sudo_cmd(pattern="ffmpegsave$", allow_sudo=True))
...
@icssbot.on(admin_cmd(pattern="vtrim"))
@icssbot.on(sudo_cmd(pattern="vtrim", allow_sudo=True))
async def ff_mpeg_trim_cmd(event):
    if event.fwd_from:
        return
    if not os.path.exists(FF_MPEG_DOWN_LOAD_MEDIA_PATH):
...
@icssbot.on(admin_cmd(pattern="atrim"))
@icssbot.on(sudo_cmd(pattern="atrim", allow_sudo=True))
async def ff_mpeg_trim_cmd(event):
    if event.fwd_from:
        return
    if not os.path.exists(FF_MPEG_DOWN_LOAD_MEDIA_PATH):
...
@icssbot.on(admin_cmd(pattern="ffmpegclear$"))
@icssbot.on(sudo_cmd(pattern="ffmpegclear$", allow_sudo=True))
async def ff_mpeg_trim_cmd(event):
    if event.fwd_from:
        return
    if not os.path.exists(FF_MPEG_DOWN_LOAD_MEDIA_PATH):
