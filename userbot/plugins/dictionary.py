# Urban Dictionary for Strip edit by @ModLso
from PyDictionary import PyDictionary

from . import AioHttp


@Stripbot.on(admin_cmd(pattern="ud (.*)"))
@Stripbot.on(sudo_cmd(pattern="ud (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@Stripbot.on(admin_cmd(pattern="معنى (.*)"))
@Stripbot.on(sudo_cmd(pattern="معنى (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    word = event.pattern_match.group(1)
